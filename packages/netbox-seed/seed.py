...
site, _ = Site.objects.update_or_create(slug="home", defaults={"name": "home", "status": "active"})
...
for port in ["eth1", "lan1", "lan2", "lan3", "lan4", "lan5", "br-lan", "br-servers", "br-iot", "br-guest"]:
...
ip("fd8a:2e59:7bfd:20::1/64", iface(router, "br-servers"), "openwrt.servers.lan", "anycast")

ap = device("openwrt-ap", "access-point", "generic-ap")
ip("192.168.10.2/24", iface(ap, "lan", "64:DD:68:37:2A:32", "1000base-t"), "openwrt-ap.lan")

nixbox = device("nixbox", "server", "generic-server")
ip("192.168.20.200/24", iface(nixbox, "bond0", "4A:C9:5C:CB:BA:93", "lag"), "nixbox.lan")
...
ip("192.168.20.205/24", iface(bmc, "mgmt", "9C:6B:00:A9:15:CC", "1000base-t", True), "nixbox-bmc.lan")

nixworker = device("nixworker", "server", "generic-server")
ip("192.168.20.210/24", iface(nixworker, "bond0", "AE:60:4D:85:25:DD", "lag"), "nixworker.lan")
...
ip("192.168.10.100/24", iface(simon_desktop, "enp14s0", "D8:43:AE:3E:5F:73", "1000base-t"), "simon-desktop.lan")
...
ip("192.168.10.50/24", iface(ha, "lan", "20:F8:3B:01:57:AB", "1000base-t"), "homeassistant.lan")
...
ip("192.168.10.153/24", iface(printer, "lan", "E0:73:E7:F4:DB:55", "1000base-t"), "HPF4DB54.lan")
